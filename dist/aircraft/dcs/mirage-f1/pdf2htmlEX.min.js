...
function Page(page) {
  ...
  // content box
  var content_box = page.getElementsByClassName(
  ...
  if (content_box) {
    ...
    this.original_scale = this.cur_scale =
      this.original_height / content_box.clientHeight;
...
Page.prototype = {
  ...
  view_position: function () {
    ...
    var c = p.parentNode;
    return [
      c.scrollLeft - p.offsetLeft - p.clientLeft,
...
Viewer.prototype = {
  ...
  init_after_loading_content: function () {
    ...
    disable_dragstart(
      document.getElementsByClassName(CSS_CLASS_NAMES.background_image)
    );
  ...
  find_pages: function () {
    ...
    for (var i = 0, l = nodes.length; i < l; ++i) {
      var cur_node = nodes[i];
      if (
        cur_node.nodeType === Node.ELEMENT_NODE &&
  ...
  load_page: function (idx, pages_to_preload, callback) {
    ...
    if (url) {
      ...
      var new_loading_indicator = cur_page_ele.getElementsByClassName(
        this.config["loading_indicator_cls"]
      )[0];
      ...
      // load data
      {
        ...
        xhr.open(
          "GET",
          `https://assets.chucksguides.com/aircraft/dcs/mirage-f1/19f38f7a899b8f12d99bec2c494e170b/${url}`,
          true
        );
        xhr.onload = function () {
          if (xhr.status === 200 || xhr.status === 0) {
            ...
            disable_dragstart(
              new_page.getElementsByClassName(CSS_CLASS_NAMES.background_image)
            );
  ...
  pre_hide_pages: function () {
    ...
    var s =
      "@media screen{." + CSS_CLASS_NAMES.page_content_box + "{display:none;}}";
  ...
  update_page_idx: function () {
    ...
    while (rest_len > 1) {
      ...
      if (
        cur_page_ele.offsetTop +
        cur_page_ele.clientTop +
        cur_page_ele.clientHeight >=
        container_min_y
      ) {
        last_idx = idx;
      } else {
    ...
    for (var i = last_idx; i < pages_len; ++i) {
      ...
      var page_visible_ratio =
        (Math.min(container_max_y, page_max_y) -
    ...
    window.addEventListener(
      ...
      function (e) {
        if (e.ctrlKey) {
          ...
          self.rescale(
            Math.pow(self.config["scale_step"], e.detail),
            true,
            fixed_point
          );
        }
  ...
  rescale: function (ratio, is_relative, fixed_point) {
    ...
    if (ratio === 0) {
      new_scale = 1;
      is_relative = false;
    } else if (is_relative) new_scale *= ratio;
    else new_scale = ratio;
    ...
    if (fp_x_inside < 0) fp_x_inside = 0;
    else if (fp_x_inside > fp_p_width) fp_x_inside = fp_p_width;

    var fp_y_ref = fp_p.offsetTop + fp_p.clientTop;
    var fp_y_inside = fixed_point[1] - fp_y_ref;
    if (fp_y_inside < 0) fp_y_inside = 0;
    else if (fp_y_inside > fp_p_height) fp_y_inside = fp_p_height;

    // Rescale pages
    for (var i = 0; i < pl_len; ++i) pl[i].rescale(new_scale);

    // Correct container scroll to keep view aligned while zooming
    container.scrollLeft +=
      (fp_x_inside / old_scale) * new_scale +
      fp_p.offsetLeft +
      fp_p.clientLeft -
      fp_x_inside -
      fp_x_ref;
    container.scrollTop +=
      (fp_y_inside / old_scale) * new_scale +
      fp_p.offsetTop +
      fp_p.clientTop -
      fp_y_inside -
      fp_y_ref;
  ...
  fit_width: function () {
    ...
    this.rescale(
      this.container.clientWidth / this.pages[page_idx].width(),
      true
    );
  ...
  fit_height: function () {
    var page_idx = this.cur_page_idx;
    this.rescale(
      this.container.clientHeight / this.pages[page_idx].height(),
      true
    );
    this.scroll_to(page_idx);
  },
  ...
  link_handler: function (e) {
    ...
    var detail_str = /** @type{string} */ (
      target.getAttribute("data-dest-detail")
    );
  ...
  navigate_to_dest: function (detail_str, src_page) {
    ...
    while (detail.length < 6) detail.push(null);
    ...
    cur_pos = transform(cur_page.ictm, [
      cur_pos[0],
      cur_page.height() - cur_pos[1],
    ]);
    ...
    switch (detail[1]) {
      case "XYZ":
        pos = [
          detail[2] === null ? cur_pos[0] : detail[2] * scale,
