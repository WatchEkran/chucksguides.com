...
const aircraftArray = [];
Object.entries(siteConfig.games).forEach(([game, modules]) => {
...
const lookupByPdfName = lodash.keyBy(aircraftArray, ({ pdfUrl }) =>
...
export default async function getPdfMetadata(pathToPdf) {
  ...
  if (aircraftData) {
    console.log(
      `Found aircraft config in aircraft-data.json for filename '${filename}'`
    );
    console.log(metadata);
  } else {
    console.warn(
      `Could not find aircraft config in aircraft-data.json for filename '${filename}'`
    );
  }
